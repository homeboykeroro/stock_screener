...
from constant.runtime_indicator import RuntimeIndicator
...
def get_upside_change_boolean_df(
    ...
    if isinstance(unusual_upside_indicators, list):
        ...
        close_pct_change_df = src_df.loc[:, idx[:, CustomisedIndicator.CLOSE_CHANGE]].rename(columns={CustomisedIndicator.CLOSE_CHANGE: 'Candlestick'})
...
def get_candlestick_type_boolean_df(
    ...
    open_df = src_df.loc[:, idx[:, Indicator.OPEN]].rename(columns={Indicator.OPEN: 'Compare'})
    high_df = src_df.loc[:, idx[:, Indicator.HIGH]].rename(columns={Indicator.HIGH: 'Compare'})
    low_df = src_df.loc[:, idx[:, Indicator.LOW]].rename(columns={Indicator.LOW: 'Compare'})
    close_df = src_df.loc[:, idx[:, Indicator.CLOSE]].rename(columns={Indicator.CLOSE: 'Compare'})

    high_pct_change_df = src_df.loc[:, idx[:, 'High Change']].rename(columns={'High Change': 'Compare'})
    close_pct_change_df = src_df.loc[:, idx[:, CustomisedIndicator.CLOSE_CHANGE]].rename(columns={CustomisedIndicator.CLOSE_CHANGE: 'Compare'})
...
def get_increasing_vol_boolean_df(src_df: DataFrame, increasing_vol_extent: float, vol_val: int, consecutive_day: int):
    vol_df = src_df.loc[:, idx[:, Indicator.VOLUME]].rename(columns={Indicator.VOLUME: 'Increasing Vol'})
    vol_pct_change_df = src_df.loc[:, idx[:, CustomisedIndicator.VOLUME_CHANGE]].rename(columns={CustomisedIndicator.VOLUME_CHANGE: 'Increasing Vol'})
...
def get_consolidation_df(
    ...
    if isinstance(start_idx, DataFrame):
        src_high_df = get_data_from_df_by_idx_range(historical_data_df.loc[:, idx[:, Indicator.HIGH]], start_idx, max_consolidation_range)
        src_low_df = get_data_from_df_by_idx_range(historical_data_df.loc[:, idx[:, Indicator.LOW]], start_idx, max_consolidation_range)
        src_close_df = get_data_from_df_by_idx_range(historical_data_df.loc[:, idx[:, Indicator.CLOSE]], start_idx, max_consolidation_range)
    else:
        end_index = len(historical_data_df)
        start_index = end_index - start_idx

        src_high_df = historical_data_df.loc[:, idx[:, Indicator.HIGH]].iloc[start_index:end_index]
        src_low_df = historical_data_df.loc[:, idx[:, Indicator.LOW]].iloc[start_index:end_index]
        src_close_df = historical_data_df.loc[:, idx[:, Indicator.CLOSE]].iloc[start_index:end_index]

    repeat_src_high_df = pd.DataFrame(np.repeat(src_high_df.values, len(src_high_df), axis=0), columns=src_high_df.columns, index=np.repeat(src_high_df.reset_index().index.tolist(), len(src_high_df), axis=0)).rename(columns={Indicator.HIGH: 'Compare'})
    expand_src_high_df = pd.concat([src_high_df] * len(src_high_df)).rename(columns={Indicator.HIGH: 'Compare'})
    ...
    repeat_src_low_df = pd.DataFrame(np.repeat(src_low_df.values, len(src_low_df), axis=0), columns=src_low_df.columns, index=np.repeat(src_low_df.reset_index().index.tolist(), len(src_low_df), axis=0)).rename(columns={Indicator.LOW: 'Compare'})
    expand_src_low_df = pd.concat([src_low_df] * len(src_low_df)).rename(columns={Indicator.LOW: 'Compare'})
    ...
    repeat_src_close_df = pd.DataFrame(np.repeat(src_close_df.values, len(src_close_df), axis=0), columns=src_close_df.columns, index=np.repeat(src_close_df.reset_index().index.tolist(), len(src_close_df), axis=0)).rename(columns={Indicator.CLOSE: 'Compare'})
    expand_src_close_df = pd.concat([src_close_df] * len(src_close_df)).rename(columns={Indicator.CLOSE: 'Compare'})
    ...
    indicator_to_in_range_boolean_df_dict = {
        Indicator.HIGH: high_in_range_boolean_count_df,
        Indicator.LOW: low_in_range_boolean_count_df,
        Indicator.CLOSE: close_in_range_boolean_count_df
   }
